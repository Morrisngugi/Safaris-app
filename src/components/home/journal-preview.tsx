    ...
    <section className="bg-[var(--color-ivory)] py-24">
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
        <div className="mb-12 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="mb-4 text-[0.7rem] font-medium uppercase tracking-[0.28em] text-[var(--color-gold)]">Field notes</div>
            <h2 className="font-serif text-4xl leading-none text-[var(--color-charcoal)] sm:text-6xl">Stories, guides & inspiration from Africa.</h2>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          {journalArticles.map((article) => (
            <article key={article.slug} className="group overflow-hidden border-b border-[var(--color-border)] pb-7 transition">
